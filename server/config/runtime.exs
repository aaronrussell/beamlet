...
if config_env() == :prod do
  ...
  # The address the beamlet is reached at, which is what LiveView
  # checks socket origins against and what Host.Router.url builds on.
  url = URI.parse(System.get_env("BEAMLET_URL", "http://localhost:4000"))
  ...
  unless url.scheme in ["http", "https"] and is_binary(url.host) and url.host != "" do
    ...
    got: #{inspect(System.get_env("BEAMLET_URL"))}
    """
  end
